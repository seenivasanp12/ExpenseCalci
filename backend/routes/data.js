...
router.put('/expense/:day', requireAuth, async (req, res) => {
  try {
    const { year, month, amount, remark } = req.body
    const { error } = await supabase.from('expenses').upsert(
      { user_id: req.user.id, year, month, day: Number(req.params.day), amount: Number(amount) || 0, remark: remark || '' },
